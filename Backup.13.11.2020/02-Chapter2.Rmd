...
**Creating a table with the top 6 fish species across time-periods**
...
#     code comments made at Figure 3.3. The only exception being that time-period is replaced by fishing depths.  
```
...
* $H_O$ **(Null hypothesis)** – Mean species weights are *equal* across time-periods and fishing depths. 

* $H_A$ **(Alternate hypothesis)** - Mean species weights are *not equal* across time-periods and fishing depths.
...
### Analysis of the mean discarded catch across time-Periods and fishing depths using the **Welch Two Sample t-test**.
