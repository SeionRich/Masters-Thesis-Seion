...
n <- length(res_RSA$peaks_mat)
best_scores <- sort(res_RSA$peaks_mat,partial=n-0:2)[n-0:2]
ind <- arrayInd(which(res_RSA$peaks_mat %in% best_scores),
                dim(res_RSA$peaks_mat))
Ks <- as.numeric(rownames(res_RSA$peaks_mat)[ind[,1]])
Linfs <- as.numeric(colnames(res_RSA$peaks_mat)[ind[,2]])
